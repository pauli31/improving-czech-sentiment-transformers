...
class MyTensorBoardCallback(TensorBoard):

    def __init__(self, log_dir, batch_size, **kwargs):
        super().__init__(log_dir, batch_size=batch_size, histogram_freq=1)
        dir_file = str(self.log_dir) + '/learning'
        self.lr_writer = tf.summary.create_file_writer(dir_file)
        self.global_step = 1
    ...
    def on_train_batch_end(self, batch, logs=None):
        ...
        with self.lr_writer.as_default():
            summary = tf.summary.scalar('learning_rate', lr, self.global_step)
        self.global_step += 1
...
class MyWandbCallbackLogger(Callback):
    def __init__(self,
                 print_batch_freq=1,
                 use_only_train_data=False):
        super(MyWandbCallbackLogger, self).__init__()
    ...
    def on_train_batch_end(self, batch, logs=None):
        ...
        if (batch+1) % self.print_batch_freq == 0:
            try:
                wandb.log(
                    {'epoch': self.epoch,
                     'batch': batch,
                     'global_batch': self.global_step,
                     'avg_loss': train_loss,
...
# -----------------------
# prepsanej Callback z from wandb.keras import WandbCallback
...
def patch_tf_keras():
    ...
    def set_wandb_attrs(cbk, val_data):
        if isinstance(cbk, MyOriginalWandbCallback):
...
class MyOriginalWandbCallback(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        self.perform_my_epoc_end_log(epoch, logs, commit=True)
        self.epoch += 1
    ...
    def perform_my_epoc_end_log(self, epoch, logs, commit):
        train_loss = logs['loss']
        train_acc = logs['acc']
        val_loss = val_acc = 0
        if self.use_only_train_data is False:
        ...
        try:
            wandb.log({'train_acc': train_acc,
                       'train_loss': train_loss,
                       'val_acc': val_acc,
                       'val_loss': val_loss}, commit=commit)

        except Exception as e:
            logger.error("Error WANDB with exception e:" + str(e))

    def perform_my_batch_end_log(self, batch, logs, commit):
        train_loss = logs['loss']
        train_acc = logs['acc']
        lr = getattr(self.model.optimizer, 'lr', None)
        if isinstance(lr, LearningRateSchedule):
        ...
        try:
            wandb.log(
                {'epoch': self.epoch,
                 ...
                 'avg_loss': train_loss,
                 'avg_accuracy': train_acc,
                 'current_lr': lr}, commit=commit)
        except Exception as e:
            logger.error("Error WANDB with exception e:" + str(e))


    # Tahle metoda se neprovola nevim proc
    # This is what keras used pre tensorflow.keras
    def on_batch_end(self, batch, logs=None):
        ...
        #
        #     # Moje prepsani
        #     self.perform_my_batch_log(batch, logs)
    ...
    def on_train_batch_end(self, batch, logs=None):
        ...
        if self.log_batch_frequency and batch % self.log_batch_frequency == 0:
            # wandb.log(logs, commit=True)
            self.perform_my_batch_end_log(batch, logs, commit=True)
